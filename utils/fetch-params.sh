...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/AtanapiParams"
else
    PARAMS_DIR="$HOME/.atanapi-params"
fi
...
DOWNLOAD_URL="https://github.com/Raptor3um/atanapi/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with NAPI_DISABLE_SOMETHING=1
NAPI_DISABLE_WGET="${NAPI_DISABLE_WGET:-}"
#NAPI_DISABLE_IPFS="${NAPI_DISABLE_IPFS:-}"
NAPI_DISABLE_CURL="${NAPI_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$NAPI_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$NAPI_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$NAPI_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Atanapi parameters!
...
main() {
    ...
    cat <<EOF
NAPI - fetch-params.sh

This script will fetch the Atanapi SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Atanapi SNARK parameters. Note that it is
