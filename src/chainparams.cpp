...
// Copyright (c) 2020-2022 The Atanapi developers
...
{
    const char* pszTimestamp = "ATANAPI: tomorrow is better than yesterday";
    const CScript genesisOutputScript = CScript() << ParseHex("0499eaf2f7377f7f6c248c7d7f0c983335834825f85f5eac06c65134854bb7e0f9dd571068acf561077536530c6fd70d97302c131c69c4364871126b14875ca973") << OP_CHECKSIG;
...
public:
    CMainParams() {
        ...
        consensus.nSmartnodePaymentsStartBlock = 10000;
        ...
        consensus.nPowTargetTimespan = 24 * 60 * 60; // Atanapi: 1 day
        consensus.nPowTargetSpacing = 1 * 60; // Atanapi: 1 minutes
        ...
        genesis = CreateGenesisBlock(1665017980, 424, 0x20001fff, 4, 2000 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256S("0x555a58b65bfd76ba18916ed7933cbdf7585e99c7186367c2cefe9ab2915c40e3"));
        assert(genesis.hashMerkleRoot == uint256S("0x419cc9752d9c4cb0ffb3451452b506a9d0ce3809d001151d6aa58ecb86802347"));
        ...
        // Atanapi script addresses start with 'N'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,53);
        // Atanapi private keys start with 'N' or 'P'
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,54);
        // Atanapi BIP32 pubkeys start with 'xpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x03, 0x76, 0x13, 0x78};
        // Atanapi BIP32 prvkeys start with 'xprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x03, 0x76, 0x13, 0xFD};
        // Atanapi BIP44 coin type is '5'
        std::string strExtCoinType = gArgs.GetArg("-extcoinindex", "");
        nExtCoinType = strExtCoinType.empty() ? 200 : std::stoi(strExtCoinType);
        std::vector<FounderRewardStructure> rewardStructures = {  {INT_MAX, 5} };// 5% Atanapi Project Development
        ...
        vSporkAddresses = {"AS3QVrTANUk9Uo4ePvuXiAiEnK4nfaQiWj"};
        nMinSporkKeys = 1;
        ...
        chainTxData = ChainTxData{
          1665183303,   // * UNIX timestamp of last known number of transactions (Block 0)
...
public:
    CTestNetParams() {
        ...
        consensus.powLimit = uint256S("00ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"); // ~uint256(0) >> 20
        consensus.nPowTargetTimespan = 24 * 60 * 60; // Atanapi: 1 day
        consensus.nPowTargetSpacing = 60; // Atanapi: 1 minutes
        ...
        // By default assume that the signatures in ancestors of this block are valid.
        consensus.defaultAssumeValid = uint256S("0x91aaaa67b2857bc0841c9ddcc6bc8e0511fb2f9d3845d2db9df7265be6c6a3fc");
        ...
        //FindMainNetGenesisBlock(1665061800, 0x20001fff, "test");
        genesis = CreateGenesisBlock(1665061800, 642, 0x20001fff, 4, 2000 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256S("0x91aaaa67b2857bc0841c9ddcc6bc8e0511fb2f9d3845d2db9df7265be6c6a3fc"));
        assert(genesis.hashMerkleRoot == uint256S("0x419cc9752d9c4cb0ffb3451452b506a9d0ce3809d001151d6aa58ecb86802347"));
        ...
        //vSeeds.emplace_back("seed00.atanapi.org");

        // Testnet Atanapi addresses start with 'T'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,65);
        // Testnet Atanapi script addresses start with '8' or '9'
        ...
        // Testnet Atanapi BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x03, 0x7B, 0xCF, 0xC4};
        // Testnet Atanapi BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x03, 0x7B, 0xd0, 0x49};

        // Testnet Atanapi BIP44 coin type is '1' (All coin's testnet default)
        std::string strExtCoinType = gArgs.GetArg("-extcoinindex", "");
        ...
        consensus.nFounderPayment = FounderPayment(rewardStructures, 200, "AS3QVrTANUk9Uo4ePvuXiAiEnK4nfaQiWj");
        ...
        chainTxData = ChainTxData{
            1665061800, // * UNIX timestamp of last known number of transactions (Block 213054)
            0,    // * total number of transactions between genesis and that timestamp
                        //   (the tx=... number in the SetBestChain debug.log lines)
            0        // * estimated number of transactions per second after that timestamp
        };
...
public:
    CDevNetParams() {
       ...
       // consensus.DIP0003EnforcementHeight = 2; // DIP0003 activated immediately on devnet
        ...
        consensus.nPowTargetTimespan = 24 * 60 * 60; // Atanapi: 1 day
        consensus.nPowTargetSpacing = 2.5 * 60; // Atanapi: 2.5 minutes
        consensus.fPowAllowMinDifficultyBlocks = true;
        consensus.fPowNoRetargeting = false;
        consensus.nPowDGWHeight = 60;
        consensus.DGWBlocksAvg = 60;
        ...
        vFixedSeeds.clear();
        vSeeds.clear();
        //vSeeds.push_back(CDNSSeedData("atanapievo.org",  "devnet-seed.atanapievo.org"));

        // Testnet Atanapi addresses start with 'y'
...
public:
    CRegTestParams() {
       ...
       // consensus.DIP0003EnforcementHeight = 500;
        ...
        // Regtest Atanapi addresses start with 'y'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,140);
        // Regtest Atanapi script addresses start with '8' or '9'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,19);
        // Regtest private keys start with '9' or 'c' (Bitcoin defaults)
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,239);
        // Regtest Atanapi BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCF};
        // Regtest Atanapi BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x83, 0x94};

        // Regtest Atanapi BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
